...
func init() {
	...
	flag.StringVar(&host, "host", "", "host to send reindex request")
...
func buildDictionary(sourcePath string, config suggest.IndexDescription) dictionary.Dictionary {
	...
	if err != nil {
		log.Fatalf("Fail to create dictionary file %s", err)
	}

	cdbHandle := cdb.New()
	cdbWriter, err := cdbHandle.GetWriter(destinationFile)
	if err != nil {
		log.Fatalf("Fail to create cdb writer %s", err)
	}
...
//
func tryToSendReindexRequest() {
	resp, err := http.Post(host, "text/plain", nil)
	if err != nil {
		log.Printf("Fail to send reindex request to %s, %s", host, err)
	}
	...
	if err != nil {
		log.Printf("Fail to read response body %s", err)
	}

	if string(body) != "OK" {
		log.Printf("Something goes wrong with reindex request, %s", body)
	}
}
