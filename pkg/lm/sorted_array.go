...
// CreateSortedArray creates a NGramVector form the given Tree
func CreateSortedArray(tree rbtree.Tree) NGramVector {
	var node *nGramNode
	keys := make([]uint64, 0, tree.Len())
	values := make([]WordCount, 0, tree.Len())
	total := WordCount(0)

	for iter := tree.NewIterator(); iter.Next() != nil; {
